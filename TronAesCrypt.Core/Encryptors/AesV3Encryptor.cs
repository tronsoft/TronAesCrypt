...
internal class AesV3Encryptor : IAesEncryptor
{
    ...
    public void EncryptStream(Stream inStream, Stream outStream, string password, int bufferSize, int kdfIterations = 300_000)
    {
        ...
        using var key = new SensitiveData(kdf.DeriveKey(password, ivMainKey));
        using var internalKey = new SensitiveData(RandomSaltGenerator.Generate(32));
    ...
    /// <inheritdoc />
    public async Task EncryptStreamAsync(Stream inStream, Stream outStream, string password, int bufferSize, int kdfIterations = 300_000, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(inStream);
        ArgumentNullException.ThrowIfNull(outStream);
        ArgumentNullException.ThrowIfNull(password);
        ...
        _aesCryptHeader.WriteHeader(outStream, kdfIterations);
        await outStream.WriteAsync(ivMainKey.AsMemory(), cancellationToken).ConfigureAwait(false);
        await outStream.WriteAsync(encryptedMainKeyIv.AsMemory(), cancellationToken).ConfigureAwait(false);
        ...
        using (var hmacMainKeyIv = new HMACSHA256(key))
        {
            ...
            dataToHash[encryptedMainKeyIv.Length] = 0x03;
            var hash = hmacMainKeyIv.ComputeHash(dataToHash);
            await outStream.WriteAsync(hash.AsMemory(), cancellationToken).ConfigureAwait(false);
        }

        var hmac0Value = await EncryptDataV3Async(inStream, outStream, internalKey, ivData, bufferSize, cancellationToken).ConfigureAwait(false);

        // V3: No modulo byte, just the HMAC
        await outStream.WriteAsync(hmac0Value.AsMemory(), cancellationToken).ConfigureAwait(false);
    }
    ...
    private static async Task<byte[]> EncryptDataV3Async(Stream inStream, Stream outStream, byte[] internalKey, byte[] iv, int bufferSize, CancellationToken cancellationToken)
    {
        using var cipher = AesFactory.Create(internalKey, iv, usePkcs7Padding: true);
        using var hmac0 = new HMACSHA256(internalKey);
        hmac0.Initialize();

        await using var hmacStream = new HmacComputingStream(outStream, hmac0);
        await using var cryptoStream = new CryptoStream(hmacStream, cipher.CreateEncryptor(), CryptoStreamMode.Write, leaveOpen: true);
        var buffer = ArrayPool<byte>.Shared.Rent(bufferSize);
        try
        {
            int bytesRead;
            while ((bytesRead = await inStream.ReadAsync(buffer.AsMemory(0, bufferSize), cancellationToken).ConfigureAwait(false)) > 0)
            {
