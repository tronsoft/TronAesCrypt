...
> **Note**: Decrypting from stdin buffers the entire ciphertext before processing. The implementation first writes to a temporary file on disk and only falls back to in-memory buffering if temp-file creation fails. For very large files, prefer file-based decryption.
